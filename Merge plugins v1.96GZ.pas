{
  Merge Plugins Script v1.9.6 GZ - Unofficial Patch
...
// edited for v1.96 2015-05-4 GalenZ
procedure CopyGeneralAssets(filename: string; MOActiveMods: TStringList);
var
  ignore: TStringList;
  // rec: TSearchRec;   // not needed for 1.9.6 GalenZ 2014-05-04
...
begin
  ...
  { old 1.9.5 code  commented out by GalenZ 2015-05-04
  ...
  // new 1.9.6 code bv GalenZ - edited 2015-05-04
  modName := gzMOGetModContainingFile( moPath, MOActiveMods, filename);
  modPath := moPath + 'mods\' + modName;
  
  // copy assets from folder
  if (modName <> '') and (slCopiedFrom.IndexOf(modPath) = -1) then begin
...
begin
  ...
  try
    ...
    if copyAll then begin
      MOActiveMods := TStringList.Create;
      gzMOGetActiveMods( moPath, MOActiveMods);   // GalenZ 1.9.6 2015-05-04
      didNothing := true;
      ...
      for i := 0 to slMerge.Count - 1 do begin
        CopyGeneralAssets(slMerge[i], MOActiveMods);
        Application.processmessages;
        didNothing := false;
      end;
      if didNothing then
        LogMessage('    No general assets copied.');
      MOActiveMods.Free;        // GalenZ 1.9.6 2015-05-04
    end;
...
{
  gzMergePluginFuncs - GalenZ's support functions for MergePlugins 1.96GZ
  current version: 2015-05-04
  
  These functions exist to support correct asset copying when merging mods under MO.
  In MergePlugins 1.95, when merging assets for xxx,esp, MP took assets from the first folder in
  <moPath>/mods that contained an xxx.esp file, without any concern for whether that folder (mod) was
  activated in the current profile or not.   This meant that if the user had multiple mods containing
  the same .esp file, there was no guarantee that the assets would be copied from the correct folder.
  In fact, if older versions of a mod were being kept around (presumably de-activated), and those old
  versions had names that sorted before the new versions, it would be *guaranteed* that Merge Plugins would
  copy the wrong assets.
  
  1.96 fixes this by only looking at the active plugins of the active profile, and if there are multiple
  mods (folders) that contain the same esp, using the last-loading mod as the source of assets.

  Note that this is still not *entirely* correct.  If the esp is in the overwrites/ folder, current 1.9.6
  code will NOT copy assets from the overwrite folder, but will still look for assets of the last active mod
  containing the file.  This is probably a Good Thing.
...
{ ----------------------------------------------------------------------
  ...
  This list will be ordered exactly as it appears in modlists.txt, which is last-loading mods first.
  That is, in exact reverse order from how you normally view the list in MO.
  Parameters:
    moPath:  the pathname of the Mod Organizer folder, with a trailing '/'. e.g, 'c:/Mod Organizer/'
    modList: a non-null list to which the mod names will be added.  Will be emptied first.
}

procedure gzMOGetActiveMods( moPath: string; toList: TStringList);
begin
  //LogMessage( 'called gzMOGetActiveMods on ' + moPath);
  gzMOGetActiveModsInProfile( moPath, toList, gzMOGetActiveProfile( moPath));  
end;

{ ----------------------------------------------------------------------
  Get the list of Mod Organizer's currently active mods in the specified profile.
  This list will be ordered exactly as it appears in modlists.txt, which is last-loading mods first.
  That is, in exact reverse order from how you normally view the list in MO.
  Parameters:
    moPath:  the pathname of the Mod Organizer folder, with a trailing '/'. e.g, 'c:/Mod Organizer/'
    modList: a non-null list to which the mod names will be added.  Will be emptied first.
    profileName:  the name of the profile to look in, which is the folder <moPath>/profiles/<profileName>
}

procedure gzMOGetActiveModsInProfile( moPath: string; modList: TStringList; profileName: string);
var
  modlistFilePath: string;
  s: string;
  i: int;
begin
  modListFilePath := moPath + 'profiles/' + profileName + '/modlist.txt';
  //LogMessage( 'getting active mods from profile ' + modListFilePath);
  modList.Clear;
  if( FileExists( modListFilePath) ) then begin
    modList.LoadFromFile( modListFilePath);
    for i := modList.Count-1 downto 0 do begin
      s := modList[i];
      if( gzStrStartsWith(s, '+')) then begin          // if line starts with '+', then it's an active mod
        modList[i] := gzStrRemovePrefix( s, '+');   // so strip the '+' off the name
      end
      else
        modList.Delete(i);      // else it's a comment or inactive mod, so delete it
    end
  end
  else
    LogMessage( 'Cannot find modlist file ' + modListFilePath);
end;
...
{ ----------------------------------------------------------------------
  ...
  Returns:
    the value of 'active_profile' in the [General] section of ModOrganizer.ini in the moPath folder.
    or '' if it cannot be read for some reason.
}

function gzMOGetActiveProfile( moPath: string) : string;
var
  ini : TMemIniFile;
  fname : string;
begin
  fname := moPath + 'ModOrganizer.ini';
  if( not FileExists( fname) ) then begin
    LogMessage( 'MO ini file ' + fname + ' does not exist');
    Result := '';
    exit;
  end;
  ini := TMemIniFile.Create(fname);
  Result := ini.ReadString( 'General', 'selected_profile', '');
  ini.Free;    // be neat and clean up
  // AddMessage( 'gzMOGetActiveProfile -> ' + Result;
end;

{ ----------------------------------------------------------------------
  Get the name of the first mod in list of mod names whose MO folder contains the specified file.
  This is typically used to find the folder containing a specified .esp file, so that assets may be copied from that folder.
  This will return the *first* mod in the list that contains the specified file.
  However, if the modlist is obtained from a modlists.txt files, which store the modlist in reverse order
  (i.e. mods that load later, i.e. win conflicts, will appear earlier in the list),
  then if multiple mods contain the same file, this will return the one that wins the conflict. 
  Parameters:
    moPath:  the pathname of the Mod Organizer folder, with a trailing '/'. e.g, 'c:/Mod Organizer/'
    modList: a list of the mods (folder names) to search through.  Typically obtained from gzMOGetActiveMods( moPath)
    filename: the name of the file to look for, relative to a mod folder, e.g. 'foo.esp' or 'meshes/foo.nif'
  Returns:
    the name of the first <modName> in modList such that FileExists( <moPath>/mods/<modName>/<filename>)
    or '' if the file cannot be found.
}

function gzMOGetModContainingFile( moPath: string; modList: TStringList; filename: string) : string;
var
  i, n: int;
  modName: string;
  filePath: string;
begin
  ...
  for i := 0 to n-1 do begin
    modName := modList[i];
    filePath := moPath + 'mods/' + modName + '/' + filename;
...
{ ----------------------------------------------------------------------
  Removes the specified prefix from the specified string, if present.
  Examples:
    gzStrRemovePrefix( 'foobar', 'foo') -> 'bar'
    gzStrRemovePrefix( 'foobar', 'bar') -> 'foobar'
  Parameters:
    s:  the source string
    prefix: the prefix to remove, if present
}

function gzStrRemovePrefix( s, prefix: string) : string;
begin
  if( gzStrStartsWith( s, prefix) ) then begin
    Result := Delete(s, 1, Length(prefix));
  end
  else
    Result := s;
end;
...
{ ------------------------------------------------------------------------
  ...
  if gzStrStartsWith(s, 'This is') then
    AddMessage('It works!');
}

function gzStrStartsWith(s1, prefix: string): boolean;
var
  i, n1, n2: integer;
begin
  Result := (Pos( prefix, s1) = 1);
  if Pos( prefix, s1) = 1 then
    Result := 1;
end;
